...
type Borrower = {
  ...
  guardrails: { status: string; reason?: string; checks?: string[]; risk_tier?: string; requires_approval?: boolean };
...
export default function WorkspaceDetailPage({ params }: { params: { id: string } }) {
  ...
  const [explain, setExplain] = useState<{
    top_positive: Array<{ feature_name: string; shap_value: number }>;
    top_negative: Array<{ feature_name: string; shap_value: number }>;
  } | null>(null);
  ...
  useEffect(() => {
    ...
    apiGet<PaymentsPayload>(`/api/borrowers/${id}/payments`, role).then(setPayments).catch(console.error);
  }, [id, role]);
  ...
  const submitApproval = async () => {
    ...
    setSubmitMsg(null);
    try {
      const res = await apiPost<{ task_id: string; status: string; assigned_queue: string; risk_tier?: string }>(
        `/api/borrowers/${id}/submit-approval`,
        { reason: "Submitted from Collection Workspace" },
        role,
      );
      setSubmitMsg(`Submitted — task ${res.task_id.slice(0, 8)}… (${res.status}, queue: ${res.assigned_queue})`);
    } catch (e) {
  ...
  const settlementAmount = Number(
    borrower.settlement.settlement_amount || borrower.settlement.total_balance_connected_loans || 0,
  );
  const confidence = {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        actions={
          <>
            ...
            <Button size="sm" onClick={submitApproval} disabled={submitting}>
      ...
      {submitMsg && (
        <Alert variant={submitMsg.startsWith("Submitted") ? "info" : "danger"} title={submitMsg.startsWith("Submitted") ? "Approval submitted" : "Error"}>
          {submitMsg}
        </Alert>
      )}
          ...
          <dl className="space-y-0 text-sm">
            ...
            <div className="flex justify-between border-b border-slate-100 py-2">
              ...
              <dd className="font-medium tabular-nums">
                £{Number(borrower.settlement.total_balance_connected_loans).toLocaleString()}
              </dd>
            </div>
            <div className="flex justify-between border-b border-slate-100 py-2">
              <dt className="text-slate-500">Applications</dt>
              <dd>
                {borrower.applications_summary.count} ({borrower.applications_summary.latest_status || "—"})
              </dd>
            </div>
            <div className="flex justify-between border-b border-slate-100 py-2">
              <dt className="text-slate-500">Payments (6m)</dt>
              <dd className="tabular-nums">
                £{borrower.payments_summary.total_6m.toLocaleString()} ({borrower.payments_summary.count_6m})
              </dd>
            </div>
            <div className="flex justify-between border-b border-slate-100 py-2">
              <dt className="text-slate-500">Latest activity</dt>
              <dd>
                {borrower.activities_summary?.latest_type || String(borrower.settlement.latest_activity_type || "—")}
              </dd>
            </div>
      ...
      <Card>
        ...
        <p className="section-sub mb-3">
          {paymentRows.length} recent payments · £{(payments?.total_6m || 0).toLocaleString()} total
        </p>
              ...
              <tbody>
                {paymentRows.map((p, i) => (
                  <tr key={i} className="border-t border-slate-100">
                    <td className="px-3 py-2 tabular-nums">{p.payment_date || "—"}</td>
                    <td className="px-3 py-2 text-slate-600">{p.payment_type || "—"}</td>
      ...
      <Card>
        ...
        {borrower.guardrails?.risk_tier && (
          <Badge
            className="ml-2"
            variant={
              borrower.guardrails.risk_tier === "high"
                ? "danger"
                : borrower.guardrails.risk_tier === "medium"
                  ? "warning"
                  : "info"
            }
          >
            {borrower.guardrails.risk_tier} risk
          </Badge>
        )}
        <ul className="mt-2 text-xs text-slate-600 list-disc pl-4 space-y-0.5">
          {(borrower.guardrails?.checks || []).map((c) => (
            <li key={c}>{c}</li>
          ))}
        </ul>
      </Card>
      ...
      {offers && (
        <DownloadableTable
          columns={[
            { key: "recovery_rate", label: "RR" },
            { key: "installments", label: "Inst" },
            { key: "expected_value", label: "EV" },
            { key: "p_application", label: "PoAPP" },
            { key: "p_acceptance", label: "PoA" },
        ...
        >
          <div className="card overflow-hidden">
            <div className="px-4 py-3 border-b border-slate-100">
              ...
              <p className="section-sub">Full probability chain: PoAPP × PoA × PoF</p>
            </div>
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-slate-50 text-[11px] uppercase tracking-wide text-slate-500">
                  ...
                  <th className="px-4 py-2.5 text-center font-medium">PoAPP</th>
                  <th className="px-4 py-2.5 text-center font-medium">PoA</th>
                  <th className="px-4 py-2.5 text-center font-medium">PoF</th>
                </tr>
              </thead>
              <tbody>
                {offers.grid.map((g, i) => {
                  const optimal =
                    g.recovery_rate === rec?.optimal_rr && g.installments === rec?.optimal_installments;
                  return (
                    <tr key={i} className={optimal ? "bg-emerald-50/70" : "hover:bg-slate-50/80"}>
                      <td className="px-4 py-2.5">
                        {(g.recovery_rate as number) * 100}%{" "}
                        {optimal && <Badge variant="success">Optimal</Badge>}
                      </td>
                      ...
                      <td className="px-4 py-2.5 text-center font-medium tabular-nums">
                        £{Number(g.expected_value).toLocaleString()}
                      </td>
                      <td className="px-4 py-2.5 text-center tabular-nums">
                        {((g.p_application as number) * 100).toFixed(0)}%
                      </td>
                      <td className="px-4 py-2.5 text-center tabular-nums">
                        {((g.p_acceptance as number) * 100).toFixed(0)}%
                      </td>
                      <td className="px-4 py-2.5 text-center tabular-nums">
                        {((g.p_fulfillment as number) * 100).toFixed(0)}%
                      </td>
                    </tr>
                  );
                })}
