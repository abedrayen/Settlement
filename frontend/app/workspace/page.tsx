...
export default function WorkspaceIndexPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((r) => {
      ...
      if (!q) return true;
      return String(r.customer_code).includes(q) || (r.legal_name || "").toLowerCase().includes(q);
    });
  }, [rows, query, segment, status]);

  if (loading) return <LoadingState message="Loading collection workspace…" />;

  return (
    <div className="space-y-6">
      <PageHeader
        title="Collection Workspace"
        description="Search borrowers, review profiles, and submit settlement recommendations."
            ...
            <tbody>
              {filtered.length === 0 ? (
                ...
                filtered.map((r) => (
                  <tr
                    key={r.customer_code}
                    onClick={() => router.push(`/workspace/${r.customer_code}`)}
                  ...
                  >
                    ...
                    <td className="px-4 py-2.5">{r.segment ? <Badge>{r.segment}</Badge> : "—"}</td>
